...
void KBirthdayApplet::init()
{
    ...
    m_pKabcEngine = dataEngine("birthdays_namedays");
    if (m_pKabcEngine) {
...
QString KBirthdayApplet::getNamedayString(QDate date) {
  QVariant namedayStringEntry = m_pKabcEngine->query("Namedays_sk")[date.toString("MM-dd")];
